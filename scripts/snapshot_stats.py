...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--tex-out",
        help="Optional output path for a LaTeX fragment containing snapshot stats.",
    )
    parser.add_argument(
        "--less-than-eth",
        type=Decimal,
...
def write_latex_fragment(
    ...
    *,
    ...
    total_supply_wei: int,
    total_addresses: int,
    non_zero_addresses: int,
    zero_addresses: int,
    gini: float,
    less_than_eth: Decimal,
    lt_share: float,
    ...
    chain_id_str = "" if chain_id is None else str(chain_id)
    block_number_str = "" if block_number is None else str(block_number)
    total_supply_eth = format_eth_from_wei(total_supply_wei, places=6)
    less_than_eth_str = compact_decimal_string(less_than_eth)
    less_than_share_pct = f"{lt_share * 100:.2f}\\%"

    lines: list[str] = [
        "% Auto generated by scripts/snapshot_stats.py",
        "% Do not edit by hand. Re generate from balances_snapshot.json",
        "",
        f"\\newcommand{{\\SnapshotChainId}}{{{chain_id_str}}}",
        f"\\newcommand{{\\SnapshotBlockNumber}}{{{block_number_str}}}",
        f"\\newcommand{{\\SnapshotTotalSupplyETH}}{{{total_supply_eth}}}",
        f"\\newcommand{{\\SnapshotTotalAddresses}}{{{total_addresses}}}",
        f"\\newcommand{{\\SnapshotNonZeroAddresses}}{{{non_zero_addresses}}}",
        f"\\newcommand{{\\SnapshotZeroAddresses}}{{{zero_addresses}}}",
        f"\\newcommand{{\\SnapshotGini}}{{{gini:.4f}}}",
        f"\\newcommand{{\\SnapshotLessThanEthThreshold}}{{{less_than_eth_str}}}",
        f"\\newcommand{{\\SnapshotLessThanEthAddressShare}}{{{less_than_share_pct}}}",
        "",
        "\\begin{table}[h]",
        "  \\centering",
        "  \\caption{Balance concentration by threshold}",
        "  \\begin{tabular}{r r r r}",
        "    \\hline",
        "    Threshold (ETH) & Addresses & Balance (ETH) & Share of supply \\\\",
        "    \\hline",
    ]

    for row in threshold_rows:
        threshold_eth = compact_decimal_string(row["threshold_eth"])
        address_count = row["address_count"]
        total_balance_eth = format(row["total_balance_eth"], "f")
        share_pct = f"{row['share_of_supply'] * 100:.2f}\\%"
        lines.append(
            f"    {threshold_eth} & {address_count} & {total_balance_eth} & {share_pct} \\\\"
        )
...
def main() -> None:
    ...
    if args.tex_out:
        ...
        write_latex_fragment(
            ...
            gini=gini,
            less_than_eth=args.less_than_eth,
            lt_share=lt_share,
            threshold_rows=threshold_rows,
        )
        print()
        print(f"latex_fragment_written: {tex_path}")
